...
public class BonjourService : NSObject {
    ...
    public init(type: String, port: UInt16 = 0, userName: String, serviceName: String) {
        self.type = type
        self.port = port
    ...
    public func start() {
        ...
        do {
            ...
            let service = NetService(domain: "local.", type: type, name: serviceName, port: Int32(hostSocket.localPort))
            service.delegate = self
            service.publish()
            self.service = service
            print("started")
        } catch {
            print("BonjourService:Error socket.accept failed \(error)")
        }
    }
    
    public func stop() {
        print("BonjourService:stop \(self.service != nil)")
    ...
    public func send(responce: BonjourResponse, to socket: GCDAsyncSocket) {
        print("BonjourService:send \(responce)")
    ...
    public func respond(to socket: GCDAsyncSocket, context: String, result: [String:Any], statusText: String? = nil) {
        ...
        print("BonjourService:respond \(res) \(context)")
...
extension BonjourService : GCDAsyncSocketDelegate {
    public func socket(_ sock: GCDAsyncSocket, didAcceptNewSocket newSocket: GCDAsyncSocket) {
        print("BonjourService:socket:didAcceptNewSocket \(newSocket)")
    ...
    private func innerSocket(_ sock: GCDAsyncSocket, uuidSock: UUID, data: Data) {
        do {
            ...
            print("BonjourService:innerSocket \(req)")
            ...
            if let extraData = result.extraData {
                print("BonjourService  extra data \(extraData.count)")
                self.innerSocket(sock, uuidSock: uuidSock, data: extraData)
            }
        } catch {
            print("BonjourService  buffering \(data.count)")
    ...
    public func socket(_ sock: GCDAsyncSocket, didConnectToHost host: String, port: UInt16) {
        print("BonjourService:socket:didConnectToHost")
    }
    
    public func socketDidDisconnect(_ sock: GCDAsyncSocket, withError err: Error?) {
        print("BonjourService:socketDidDisconnect")
...
extension BonjourService : NetServiceDelegate {
    public func netServiceDidPublish(_ sender: NetService) {
        print("BonjourService:netServiceDidPublish")
    }
    
    public func netService(_ sender: NetService, didNotPublish errorDict: [String : NSNumber]) {
        print("BonjourService:netService:didNotPublish \(errorDict)")
    }
}
